...
/**
 * Fix Participant Counts - Complete
 * 
 * Recalculates currentParticipants for all rides based on actual CONFIRMED participations
 * Updates both the main ride record AND the club ride index record
 */

const { DynamoDBClient } = require('@aws-sdk/client-dynamodb');
const { DynamoDBDocumentClient, ScanCommand, UpdateCommand, TransactWriteCommand } = require('@aws-sdk/lib-dynamodb');
...
async function updateRideCountBoth(clubId, rideId, startDateTime, newCount) {
  const dateStr = startDateTime.split('T')[0];
  
  const items = [
    // Update main ride record
    {
      Update: {
        TableName: TABLE_NAME,
        Key: {
    ...
    // Update club ride index record
    {
      Update: {
        TableName: TABLE_NAME,
        Key: {
          PK: `CLUB#${clubId}#RIDES`,
          SK: `DATE#${dateStr}#RIDE#${rideId}`
        },
        UpdateExpression: 'SET currentParticipants = :count',
        ExpressionAttributeValues: {
          ':count': newCount
...
async function fixParticipantCounts() {
  console.log('\n=== Fixing Participant Counts (Complete) ===\n');
  ...
  for (const ride of rides) {
    ...
    if (currentCount !== confirmedCount) {
      ...
      await updateRideCountBoth(clubId, rideId, ride.startDateTime, confirmedCount);
      console.log(`  ✅ Updated both records to ${confirmedCount}\n`);
      fixed++;
    } else {
      skipped++;
    }
