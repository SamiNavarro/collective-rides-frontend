...
// services/route-file-service/handlers/upload/request-upload-url.ts
var request_upload_url_exports = {};
__export(request_upload_url_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(request_upload_url_exports);
...
var FileValidationError = class extends RouteFileError {
  constructor(message) {
...
var ALLOWED_CONTENT_TYPES = ["application/gpx+xml"];
var UPLOAD_URL_EXPIRY = 15 * 60;
  ...
  try {
    ...
    return createResponse(200, {
      ...
      data: {
        ...
        fileId,
        version,
        expiresAt: new Date(Date.now() + UPLOAD_URL_EXPIRY * 1e3).toISOString()
      }
    });
  } catch (error) {
...
async function storeFileRecord(clubId, routeId, version, fileId, request, userId) {
  ...
  const fileRecord = {
    ...
    checksum: (0, import_crypto.createHash)("sha256").update(fileId + now).digest("hex"),
    // Temporary checksum
    uploadedBy: userId,
    uploadedAt: now,
    uploadSource: "web",
