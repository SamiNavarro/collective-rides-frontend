...
// services/club-service/handlers/invitation/invite-user.ts
var invite_user_exports = {};
__export(invite_user_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(invite_user_exports);
...
async function handler(event) {
  ...
  try {
    ...
    const invitationInput = parseJsonBody(event.body);
    if (!invitationInput) {
      throw new ValidationError("Request body is required");
    }
    ...
    await authService.requireClubCapability(authContext, clubId, "invite_members" /* INVITE_MEMBERS */);
    logStructured("INFO", "Authorization successful", {
      requestId,
      userId: authContext.userId,
      clubId,
      capability: "invite_members" /* INVITE_MEMBERS */
    });
