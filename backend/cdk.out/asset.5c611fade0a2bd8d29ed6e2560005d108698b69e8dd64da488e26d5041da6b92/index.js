...
// services/club-service/handlers/membership/list-members.ts
var list_members_exports = {};
__export(list_members_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(list_members_exports);
...
var MembershipService = class {
  ...
  async joinClub(clubId, input, authContext) {
    ...
    if (existingMembership && existingMembership.status === "removed" /* REMOVED */) {
      ...
      const membership2 = await this.membershipRepository.updateMembershipStatusByClubAndUser(
        clubId,
        authContext.userId,
        "active" /* ACTIVE */,
        authContext.userId
    ...
    const initialStatus = "active" /* ACTIVE */;
...
async function handler(event) {
  ...
  try {
    ...
    let limit = MEMBERSHIP_CONSTRAINTS.DEFAULT_LIST_LIMIT;
    ...
    let role;
    if (queryParams.role) {
      if (!Object.values(ClubRole).includes(queryParams.role)) {
    ...
    if (queryParams.status) {
      if (!Object.values(MembershipStatus).includes(queryParams.status)) {
    ...
    await authService.requireClubCapability(authContext, clubId, "view_club_members" /* VIEW_CLUB_MEMBERS */);
    logStructured("INFO", "Authorization successful", {
      requestId,
      userId: authContext.userId,
      clubId,
      capability: "view_club_members" /* VIEW_CLUB_MEMBERS */
    });
    ...
    return createSuccessResponse(response, 200 /* OK */, origin);
  } catch (error) {
