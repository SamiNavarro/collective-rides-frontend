...
// services/ride-service/handlers/ride/get-ride.ts
var get_ride_exports = {};
__export(get_ride_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_ride_exports);
...
// services/ride-service/infrastructure/dynamodb-membership-helper.ts
var import_lib_dynamodb3 = require("@aws-sdk/lib-dynamodb");
var MembershipHelper = class {
  constructor(dynamoClient2, tableName2) {
    this.docClient = import_lib_dynamodb3.DynamoDBDocumentClient.from(dynamoClient2);
    this.tableName = tableName2;
  }
  /**
...
var ParticipationService = class {
  ...
  async joinRide(rideId, userId, request) {
    ...
    if (existingParticipation && existingParticipation.status !== "withdrawn" /* WITHDRAWN */ && existingParticipation.status !== "removed" /* REMOVED */) {
  ...
  async leaveRide(rideId, userId) {
    ...
    if (participation.status === "withdrawn" /* WITHDRAWN */ || participation.status === "removed" /* REMOVED */) {
      throw new ParticipationNotFoundError(`${rideId}:${userId}`);
    }
    ...
    if (previousStatus === "confirmed" /* CONFIRMED */) {
      ride.decrementParticipants();
      await this.promoteFromWaitlistIfNeeded(rideId, ride);
    } else if (previousStatus === "waitlisted" /* WAITLISTED */) {
...
var membershipHelper = new MembershipHelper(dynamoClient, tableName);
...
var handler = async (event) => {
  ...
  try {
    ...
    const viewerParticipationEntity = participants.find(
      (p) => p.userId === authContext.userId && (p.status === "confirmed" /* CONFIRMED */ || p.status === "waitlisted" /* WAITLISTED */)
    );
    const viewerParticipationData = viewerParticipationEntity?.toJSON();
    const participantsData = participants.filter((p) => p.status === "confirmed" /* CONFIRMED */).map((p) => {
    ...
    const waitlistData = participants.filter((p) => p.status === "waitlisted" /* WAITLISTED */).sort((a, b) => (a.waitlistPosition || 0) - (b.waitlistPosition || 0)).map((p) => {
      const pData = p.toJSON();
      return {
        userId: pData.userId,
        displayName: "Unknown",
    ...
    return createResponse(500, { error: "Internal server error" }, origin2);
  }
};
