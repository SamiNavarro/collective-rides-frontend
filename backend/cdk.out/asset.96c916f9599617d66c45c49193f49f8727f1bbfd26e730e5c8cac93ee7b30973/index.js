...
// services/route-file-service/handlers/processing/process-file.ts
var process_file_exports = {};
__export(process_file_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(process_file_exports);
...
var FileProcessingError = class extends RouteFileError {
...
var parseXml = (0, import_util.promisify)(import_xml2js.parseString);
...
async function processGpxFile(clubId, routeId, version) {
  try {
    ...
    throw new FileProcessingError("Failed to process GPX file", error);
  }
}
...
async function parseGpxContent(gpxContent) {
  try {
    ...
    throw new FileProcessingError("Failed to parse GPX content", error);
  }
}
...
function calculateDifficultyScore(gpxData) {
  const metadata = extractBasicMetadata(gpxData);
  const distanceDifficulty = Math.min(10, Math.max(1, metadata.totalDistance / 1e4));
  const elevationDifficulty = Math.min(10, Math.max(1, metadata.totalElevationGain / 1e3));
...
function calculatePerformanceMetrics(gpxData) {
  const metadata = extractBasicMetadata(gpxData);
  const baseTime = metadata.totalDistance / 2e4 * 60;
  const elevationTime = metadata.totalElevationGain / 100 * 10;
  return {
    estimatedTime: {
...
async function updateFileStatus(clubId, routeId, version, status, error) {
  const updateExpression = error ? "SET processingStatus = :status, processingError = :error" : "SET processingStatus = :status";
  const expressionAttributeValues = error ? { ":status": status, ":error": error } : { ":status": status };
