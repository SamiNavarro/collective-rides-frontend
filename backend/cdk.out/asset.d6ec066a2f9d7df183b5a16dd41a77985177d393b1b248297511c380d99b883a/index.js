...
// services/club-service/handlers/membership/update-member.ts
var update_member_exports = {};
__export(update_member_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(update_member_exports);
...
async function handler(event) {
  ...
  try {
    ...
    const updateInput = parseJsonBody(event);
    if (!updateInput) {
    ...
    await authService.requireClubCapability(authContext, clubId, "remove_members" /* REMOVE_MEMBERS */);
    logStructured("INFO", "Authorization successful", {
      requestId,
      userId: authContext.userId,
      clubId,
      capability: "remove_members" /* REMOVE_MEMBERS */
    });
